...
class ScheduleRepository(private val scheduleDao: ScheduleDao) {
    ...
    // @param date: Long毫秒时间戳(当天零点), 如 1738972800000L 代表 2026-02-09 00:00:00
    //
    ...
    //   A: date=1738972800000L(2026-02-09 周一), repeatRule=null,    title="小明生日"  → 普通日程
    //   B: date=1738368000000L(2026-02-02 周一), repeatRule=WEEKLY,  title="健身房"    → 每周一
    //   C: date=1739520000000L(2026-02-15),      repeatRule=MONTHLY, title="发工资"    → 每月15号
    //
    // 用户点击 2026-02-09(周一), 传入 date=1738972800000L:
    //   DAO 粗筛 → A(date匹配), B(有repeatRule且date<=), C(有repeatRule且date<=) → 3条
    //   精筛:
    //     A: repeatRule=null → 保留
    //     B: WEEKLY.matches(1738368000000L, 1738972800000L) → 都是周一 "1"=="1" → 保留
    //     C: MONTHLY.matches(1739520000000L, 1738972800000L) → "15"!="09" → 过滤掉
    //   最终结果: A + B = 2条
